...
The product is no longer gated by an activation key. Subscriptions are handled by a separate billing
portal (the TUI's `$` key opens it); the in-cluster mesh exchanges the subscription key for a
short-lived mesh key. The former offline/online activation paths and the standalone
`PlaceContext.Licensing` server have been removed.
