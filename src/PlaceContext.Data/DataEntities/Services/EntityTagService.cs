...
public sealed class EntityTagService
{
    ...
    public async Task TagRunOutputAsync(
        ...
        IReadOnlyList<RunDocumentContent> documents,
        CancellationToken ct = default)
    {
        try
        {
            var artifactValues = CollectStrings(primaryOutput);
            var corpus = BuildDocumentCorpus(documents, _docText);
            if (artifactValues.Count == 0 && corpus.Length == 0) return;

            var entities = await _entities.ListForProjectAsync(projectId, ct);
            ...
            foreach (var entity in entities)
            {
                ...
                foreach (var tag in entity.Tags)
                {
                    ...
                    if (artifactValues.Contains(tag)
                        ...
                        found.Add(new EntityTag(projectId, entity.Id, entity.Name, tag, runId, jobId));
                }
                ...
                foreach (var column in KeyColumns(entity))
                {
                    ...
                    try
                    {
                        var r = await _store.ExecuteAsync(projectId,
                    ...
                    foreach (var row in rows)
                    {
                        ...
                        if (artifactValues.Contains(key)
                            ...
                            found.Add(new EntityTag(projectId, entity.Id, entity.Name, key, runId, jobId));
                    }
                }
            }
            ...
            if (found.Count > 0)
            {
                ...
                _log?.LogInformation("Tagged run {RunId} with {Count} entity key(s).", runId, found.Count);
            }
        }
        catch (Exception ex)
        {
            _log?.LogWarning(ex, "Entity tagging failed for run {RunId} — the run itself is unaffected.", runId);
        }
    }
    ...
    private static string BuildDocumentCorpus(
        IEnumerable<RunDocumentContent> documents,
        IDocumentTextExtractor? docText)
    {
        var sb = new System.Text.StringBuilder();
        foreach (var artifact in documents)
        {
    ...
    private static HashSet<string> CollectStrings(string? primary)
    {
