...
public static class JsonFlattener
{
    ...
    /// </summary>
    public static IReadOnlyList<FlattenedJsonLeaf> Flatten(JsonElement root)
    {
        var leaves = new List<FlattenedJsonLeaf>();
        Walk(root, "", leaves);
        return leaves;
    }

    private static void Walk(JsonElement el, string path, List<FlattenedJsonLeaf> leaves)
    {
        if (el.ValueKind == JsonValueKind.Object)
        {
            ...
            if (first) leaves.Add(new FlattenedJsonLeaf(path, el)); // empty object {} is a leaf
            return;
        }
        leaves.Add(new FlattenedJsonLeaf(path, el));
    }
