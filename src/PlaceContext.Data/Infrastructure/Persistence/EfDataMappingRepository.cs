...
public sealed class EfDataMappingRepository : IDataMappingRepository
{
    ...
    {
        var fields = (JsonSerializer.Deserialize<List<DataMappingFieldJson>>(row.FieldsJson, Json) ?? new List<DataMappingFieldJson>())
    ...
    private static string SerializeFields(IReadOnlyList<DataFieldMapping> fields)
        => JsonSerializer.Serialize(fields.Select(f => new DataMappingFieldJson(f.SourcePath, f.Column, f.Type)).ToList(), Json);
