...
public sealed class EfDataEntityRepository : IDataEntityRepository
{
    ...
    {
        var relations = (JsonSerializer.Deserialize<List<DataEntityRelationJson>>(row.RelationsJson, Json) ?? new())
    ...
    private static string SerializeRelations(IReadOnlyList<EntityRelation> relations)
        => JsonSerializer.Serialize(relations.Select(r => new DataEntityRelationJson(r.Column, r.TargetEntity, r.TargetColumn)).ToList(), Json);
