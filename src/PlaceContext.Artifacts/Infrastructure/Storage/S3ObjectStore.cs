...
/// <summary>
/// <see cref="IObjectStore"/> backed by any S3-compatible store (MinIO, DigitalOcean Spaces, AWS S3)
/// via the AWS SDK. Used to persist post-job artifacts (HTML reports, charts, CSVs, raw bundles).
/// Objects are encrypted at rest with <see cref="IDataEncryptor"/>.
...
public sealed class S3ObjectStore : IObjectStore, IDisposable
{
    ...
    private static string P => DataEncryptionPurpose.ObjectStore;

    public S3ObjectStore(IOptions<ObjectStoreOptions> options, IDataEncryptor enc)
    {
        _o = options.Value;
        _enc = enc;
        if (IsEnabled)
    ...
    public async Task EnsureBucketAsync(string bucket, CancellationToken ct)
    {
        ...
        // Skip DoesS3BucketExistV2Async — it requires s3:ListBucket which managed S3 services
        // (DO Spaces) may not grant. PutBucketAsync is idempotent but may also be denied when
        // the bucket already exists and the creds lack s3:PutBucket — treat Access Denied as
        // "bucket likely exists" and let the downstream Put/Get surface real errors.
