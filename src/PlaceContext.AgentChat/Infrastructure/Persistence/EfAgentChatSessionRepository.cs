...
{
    private readonly AgentChatDbContext _db;
    private static readonly JsonSerializerOptions Json = new() { WriteIndented = false };

    public EfAgentChatSessionRepository(AgentChatDbContext db) => _db = db;
    ...
    private static AgentChatSession ToDomain(AgentChatSessionRow r)
    {
        var messages = string.IsNullOrEmpty(r.MessagesJson)
            ? new List<PersistedAgentMessageJson>()
            : JsonSerializer.Deserialize<List<PersistedAgentMessageJson>>(r.MessagesJson, Json)
              ?? new List<PersistedAgentMessageJson>();
