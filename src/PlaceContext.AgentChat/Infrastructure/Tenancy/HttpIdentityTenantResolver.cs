...
namespace PlaceContext.AgentChat.Infrastructure.Tenancy;
    ...
    IConfiguration configuration) : IRequestTenantResolver
{
    ...
    {
        var origin = configuration["PlaceContext:AgentChat:Identity:BaseAddress"]
            ?? configuration["PlaceContext:Microservices:Destinations:Identity"]
            ?? throw new InvalidOperationException(
                "Configure PlaceContext:AgentChat:Identity:BaseAddress for tenant resolution.");
    ...
    {
        var configuredToken = configuration["PlaceContext:AgentChat:Identity:ServiceToken"];
        ...
        {
            throw new InvalidOperationException(
                "AgentChat requires an Identity service token or a ServiceAuth signing key.");
        }
