...
public sealed class EfCrmCommunicationRepository : ICrmCommunicationRepository
{
    ...
    private CrmCommunicationRow ToRow(CrmCommunication value) => new()
    {
        ...
        BodyProtected = _encryptor.Protect(value.Body, IDataEncryptor.Purpose.CrmCommunication),
    ...
    private CrmCommunication ToDomain(CrmCommunicationRow row)
        => CrmCommunication.Rehydrate(
            ...
            _encryptor.Unprotect(row.BodyProtected, IDataEncryptor.Purpose.CrmCommunication),
    ...
    private string? Protect(string? value)
        => value is null ? null : _encryptor.Protect(value, IDataEncryptor.Purpose.CrmCommunication);

    private string? Unprotect(string? value)
        => value is null ? null : _encryptor.Unprotect(value, IDataEncryptor.Purpose.CrmCommunication);
}
