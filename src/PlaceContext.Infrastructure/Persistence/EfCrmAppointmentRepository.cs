...
public sealed class EfCrmAppointmentRepository : ICrmAppointmentRepository
{
    ...
    private string? Protect(string? value) => value is null ? null : _encryptor.Protect(value, IDataEncryptor.Purpose.CrmAppointment);
    private string? Unprotect(string? value) => value is null ? null : _encryptor.Unprotect(value, IDataEncryptor.Purpose.CrmAppointment);
}
