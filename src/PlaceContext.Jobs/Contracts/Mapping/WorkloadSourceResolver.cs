...
/// </summary>
public static class WorkloadSourceResolver
{
    /// <summary>
        ...
        string parameterContext)
    {
        ...
        if (hasImage)
            return new ImageWorkload(image!);
        ...
        if (hasCode)
        {
            ...
            if (hasFiles)
            {
                ...
                return new CodeWorkload(runtimeId!, codeFiles, entrypoint);
            }

            if (string.IsNullOrWhiteSpace(source))
                throw new ArgumentException($"{parameterContext}: Source (or Files) is required for code workloads.");
            return new CodeWorkload(runtimeId!, source!, entrypoint);
        }
