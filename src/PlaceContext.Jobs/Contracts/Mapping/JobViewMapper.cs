...
public static class JobViewMapper
{
    public static JobView ToView(Job job) => new(
        ...
        MapImage: (job.MapSpec.Source as ImageWorkload)?.Image,
        MapRuntimeId: (job.MapSpec.Source as CodeWorkload)?.RuntimeId,
        MapSource: (job.MapSpec.Source as CodeWorkload)?.Source,
        MapEntrypoint: (job.MapSpec.Source as CodeWorkload)?.Entrypoint,
        ...
        ReduceImage: (job.ReduceSpec?.Source as ImageWorkload)?.Image,
        ReduceRuntimeId: (job.ReduceSpec?.Source as CodeWorkload)?.RuntimeId,
        ReduceSource: (job.ReduceSpec?.Source as CodeWorkload)?.Source,
        ReduceEntrypoint: (job.ReduceSpec?.Source as CodeWorkload)?.Entrypoint,
        ...
        Parameters: job.Parameters.Select(p => new JobParameterDto(p.Name, p.Label, p.Required, p.Type, p.Options.ToList())).ToList(),
