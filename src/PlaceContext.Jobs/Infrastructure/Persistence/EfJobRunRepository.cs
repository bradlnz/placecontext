...
public sealed class EfJobRunRepository : IJobRunRepository
{
    ...
    private JobRunRow ToRow(JobRun run) => new()
    {
        ...
        ShardResultsJson = Enc(JsonSerializer.Serialize(
            run.ShardResults.Select(s => new JobRunShardResultJson
            {
                Index = s.Index,
                ExitCode = s.ExitCode,
        ...
        ReduceResultJson = run.ReduceResult is { } r
            ? Enc(JsonSerializer.Serialize(new JobRunReduceResultJson
            {
                ExitCode = r.ExitCode,
            ...
            : null,
        SnapshotJson = Enc(JsonSerializer.Serialize(ToJobRunSnapshotJson(run.Snapshot), Json)),
    };
    ...
    private JobRun ToDomain(JobRunRow row)
    {
        ...
        {
            var shards = JsonSerializer.Deserialize<List<JobRunShardResultJson>>(Dec(row.ShardResultsJson), Json)
                         ?? new List<JobRunShardResultJson>();
        ...
        {
            var rj = JsonSerializer.Deserialize<JobRunReduceResultJson>(Dec(row.ReduceResultJson), Json);
            if (rj is not null)
    ...
    private static List<JobRunArtifactJson> ToArtifactsJson(IReadOnlyList<RunArtifact> artifacts)
        => artifacts.Select(a => new JobRunArtifactJson { Name = a.Name, Content = a.Content, IsBinary = a.IsBinary ? true : null }).ToList();

    private static IReadOnlyList<RunArtifact> FromArtifactsJson(List<JobRunArtifactJson>? artifacts)
    ...
    private static JobRunSnapshotJson ToJobRunSnapshotJson(WorkloadSnapshot snap) => new()
    {
        MapSource = ToJobRunSourceJson(snap.MapSource),
        InputPayloads = snap.InputPayloads.ToList(),
        MapEnv = new Dictionary<string, string>(snap.MapEnv),
        ReduceSource = snap.ReduceSource is not null ? ToJobRunSourceJson(snap.ReduceSource) : null,
    ...
    private static WorkloadSnapshot DeserialiseSnapshot(string json)
    {
        if (string.IsNullOrWhiteSpace(json) || json == "{}")
        {
            ...
            var fallbackMap = new ImageWorkload("unknown");
            return new WorkloadSnapshot(fallbackMap, Array.Empty<string>(),
                new Dictionary<string, string>(), null, null, 1);
        }

        var sj = JsonSerializer.Deserialize<JobRunSnapshotJson>(json, Json) ?? new JobRunSnapshotJson();
        var mapSource = DeserialiseJobRunSourceJson(sj.MapSource);
        var reduceSource = sj.ReduceSource is not null ? DeserialiseJobRunSourceJson(sj.ReduceSource) : null;
    ...
    private static JobRunSourceJson ToJobRunSourceJson(WorkloadSource source) => source switch
    {
        ImageWorkload img => new JobRunSourceJson { Kind = "image", Image = img.Image },
        CodeWorkload code => new JobRunSourceJson
        {
            Kind = "code", RuntimeId = code.RuntimeId, Source = code.Source, Entrypoint = code.Entrypoint,
            Files = code.Files.Select(f => new JobRunCodeFileJson { Path = f.Path, Content = f.Content }).ToList(),
        },
        _ => throw new InvalidOperationException($"Unknown WorkloadSource: {source.GetType().Name}"),
    };

    private static WorkloadSource DeserialiseJobRunSourceJson(JobRunSourceJson sj) => sj.Kind switch
    {
        "image" => new ImageWorkload(sj.Image ?? "unknown"),
        "code" => sj.Files is { Count: > 0 }
            ? new CodeWorkload(
                sj.RuntimeId ?? "unknown",
                sj.Files.Select(f => new CodeFile(f.Path, f.Content)).ToList(),
                sj.Entrypoint)
            : new CodeWorkload(sj.RuntimeId ?? "unknown", sj.Source ?? "", sj.Entrypoint),
        _ => new ImageWorkload(sj.Image ?? "unknown"),
    };
