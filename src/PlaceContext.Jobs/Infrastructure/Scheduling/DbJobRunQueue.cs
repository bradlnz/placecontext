...
/// is added to the caller's <see cref="JobsDbContext"/> so it commits in the same transaction as the
...
public sealed class DbJobRunQueue : IJobRunQueue
{
    ...
    private static string P => DataEncryptionPurpose.PendingRun;

    public DbJobRunQueue(JobsDbContext db, IClock clock, IDataEncryptor enc)
    {
        _db = db;
        _clock = clock;
        _enc = enc;
