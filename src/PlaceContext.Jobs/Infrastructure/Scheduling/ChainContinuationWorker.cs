...
public sealed class ChainContinuationWorker : BackgroundService
{
    ...
    private async Task<IReadOnlyList<ChainContinuation>> ClaimAsync(CancellationToken ct)
    {
        ...
        return rows.Select(row => new ChainContinuation(
            row.Id, row.TenantId, row.ChainId, row.ResumeStageIndex!.Value)).ToList();
    }

    private async Task ResumeAsync(ChainContinuation value, CancellationToken ct)
    {
        try
        {
            ...
            try
            {
                ...
                var payload = persisted.FinalOutput is null ? null : encryptor.Unprotect(
                    persisted.FinalOutput, Application.Ports.DataEncryptionPurpose.ChainRun);
                ...
                if (persisted.ContinuationOverrides is { } protectedOverrides)
                {
                    var json = encryptor.Unprotect(
                        protectedOverrides, Application.Ports.DataEncryptionPurpose.ChainRun);
