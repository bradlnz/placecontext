...
public sealed record ConditionGate(string Expression, IReadOnlyList<ChainStage>? ElseBranch = null) : ChainGate
{
    ...
    internal static bool EvaluateExpression(string expression, string? payload)
    {
        ...
        try
        {
            ...
            return op switch
            {
                ...
                "contains" => EvaluateText(root, rest, ConditionTextComparison.Contains),
                "starts" => EvaluateText(root, rest, ConditionTextComparison.StartsWith),
                "ends" => EvaluateText(root, rest, ConditionTextComparison.EndsWith),
    ...
    private static bool EvaluateText(JsonElement root, string rest, ConditionTextComparison comparison)
    {
        if (!TrySplitOperand(rest, out var path, out var expected)) return false;
        var el = ResolvePath(root, path);
        if (el is null) return false;

        if (comparison == ConditionTextComparison.Contains && el.Value.ValueKind == JsonValueKind.Array)
        ...
        {
            ConditionTextComparison.Contains => actual.Contains(expected, StringComparison.OrdinalIgnoreCase),
            ConditionTextComparison.StartsWith => actual.StartsWith(expected, StringComparison.OrdinalIgnoreCase),
            ConditionTextComparison.EndsWith => actual.EndsWith(expected, StringComparison.OrdinalIgnoreCase),
            _ => false,
        };
    }
