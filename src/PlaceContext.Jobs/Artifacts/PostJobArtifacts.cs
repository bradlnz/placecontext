...
public static class PostJobArtifacts
{
    ...
    public static PostJobBuiltFile HtmlReport(Job job, JobRun run)
    {
        ...
        sb.Append("</body></html>");
        return new PostJobBuiltFile("report.html", Bytes(sb), "text/html; charset=utf-8", "HTML report");
    }
    ...
    public static PostJobBuiltFile Chart(Job job, JobRun run)
    {
        var sb = new StringBuilder();
        ...
        sb.Append("</body></html>");
        return new PostJobBuiltFile("chart.html", Bytes(sb), "text/html; charset=utf-8", "Chart");
    }

    // ── CSV ─────────────────────────────────────────────────────────────────────────────────────────
    public static PostJobBuiltFile Csv(JobRun run)
    {
        var sb = new StringBuilder();
        ...
        return new PostJobBuiltFile("run.csv", Bytes(sb), "text/csv; charset=utf-8", "CSV export");
    }

    // ── Raw bundle: each produced file, as-is ───────────────────────────────────────────────────────
    public static IEnumerable<PostJobBuiltFile> RawBundle(JobRun run)
    {
        foreach (var s in run.ShardResults)
        {
    ...
    // ── helpers ─────────────────────────────────────────────────────────────────────────────────────
    private static PostJobBuiltFile RawFile(string path, string content)
