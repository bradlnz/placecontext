...
/// global document plus the project's own, defined in the portal), so the
...
public sealed class PlaceContextPrompts
{
        ...
        [Description("The project's GUID id")] string projectId)
    {
        ...
        Record the change you just made through PlaceContext's `record_activity` tool, satisfying every
        trust gate:
        ...
        [Description("The project's GUID id")] string projectId)
    {
        ...
        with `get_project_overview`.

        ## Requirements you must follow
        {{requirements}}

        Acknowledge the key constraints, then ask what to work on (or proceed with the stated task).
        Record decisions with `add_decision`, and route every change through `record_activity`.
        """;
    }
