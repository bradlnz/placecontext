...
public sealed partial class ChatViewModel
{
    ...
    private static void RecoverLostMapCalls(AgentMessage msg)
    {
        ...
        if (!msg.Content.Contains($"{AgentToolNames.ToolCallPrefix}{AgentToolNames.RenderMap}", StringComparison.Ordinal)) return;
        foreach (var call in ScanToolCalls(msg.Content).Where(c => c.Name == AgentToolNames.RenderMap))
        {
