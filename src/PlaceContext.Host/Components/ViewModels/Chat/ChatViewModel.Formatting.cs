...
public sealed partial class ChatViewModel
{
    ...
    internal static List<(string Name, string Args, int Start, int Length)> ScanToolCalls(string text)
    {
        ...
        while (pos < text.Length)
        {
            ...
            var nextCall = text.IndexOf(AgentToolNames.ToolCallPrefix, nameStart, StringComparison.Ordinal);
            ...
            var close = searchEnd - pipe - 1 > 0
                ? text.LastIndexOf(AgentToolNames.ToolCallSuffix, searchEnd - 1, searchEnd - pipe - 1, StringComparison.Ordinal) : -1;
    ...
    {
        var getArtifactCalls = toolCalls.Where(tc => tc.ToolName == AgentToolNames.GetArtifacts).ToList();
        if (getArtifactCalls.Count == 0 || toolCalls.Any(tc => tc.ToolName == AgentToolNames.ShowArtifact)) return new() { Detected = false };
    ...
    private static HallucinationResult DetectErrorMaskingHallucination(List<ToolCallInfo> toolCalls, string content)
    {
        var failedCalls = toolCalls.Where(tc =>
            (tc.Status == AgentToolCallStatus.Error || (tc.Result != null && tc.Result.StartsWith("Error:"))) && tc.ToolName != AgentToolNames.ShowArtifact).ToList();
