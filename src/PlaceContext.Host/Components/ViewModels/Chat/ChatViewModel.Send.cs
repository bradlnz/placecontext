...
public sealed partial class ChatViewModel
{
    ...
    public async Task SendAsync(Func<Task> scrollToBottom, Func<Task> scrollAfterRender)
    {
        ...
        if (!string.IsNullOrEmpty(text) && text.StartsWith("/"))
        {
            ...
            {
                var toolArgs =
                    string.IsNullOrEmpty(match.Args) ? cmdArgs
        ...
        {
            var parsedText =
                AttachedFileText ?? "[No readable text could be extracted from this attachment.]";
        ...
        var ragTask = _ragEnabled
            ? _contextBuilder
                .BuildContextAsync(ProjectId.Value, requestText, _maxContextChunks)
        ...
        try
        {
            ...
            while (round < maxToolRounds)
            {
                ...
                if (toolCalls.Count == 0)
                {
                    if (string.IsNullOrWhiteSpace(StreamBuffer))
                        StreamBuffer =
                            "I've processed your request but didn't generate a text response. The tool results are shown above.";
                    break;
                }
                ...
                foreach (var tc in toolCalls)
                {
                    ...
                    var isJobTool =
                        tc.ToolName is AgentToolNames.RunJob or AgentToolNames.RunJobChain;
                    ...
                    for (var attempt = 0; attempt <= maxRetries; attempt++)
                    {
                        ...
                        {
                            result = ToolCallResult.Fail(
                                $"Timed out after {(int)timeout.TotalSeconds}s"
                            );
                        }
                    ...
                    tc.Status = result.Success
                        ? AgentToolCallStatus.Completed
                        : AgentToolCallStatus.Error;
                    tc.Result = result.Data ?? result.Error;
                    tc.ResultType =
                        result.IsMap ? "map"
                        : result.IsGraph ? "graph"
                        : result.IsArtifact ? "artifact"
                        : "text";

                    var action = ActiveActions.FirstOrDefault(a =>
                        a.ToolName == tc.ToolName && a.Status == AgentToolCallStatus.Running
                    );
                    if (action != null)
                    {
                ...
                if (
                    ...
                    && toolCalls.Count == 0
                    && !IsAllReasoning(rawCleaned)
                )
                    textContent = FriendlyLoadingQuip();

                var assistantMsg = new AgentMessage("assistant", textContent);
                assistantMsg.Thinking = string.IsNullOrWhiteSpace(split.Thinking)
                    ? null
                    : split.Thinking;
            ...
            if (Messages.Count == 0 || Messages.Last().Role != "assistant")
            {
                ...
                var finalContent = string.IsNullOrWhiteSpace(finalSplit.Answer)
                    ? FriendlyLoadingQuip()
                    : finalSplit.Answer;
                var finalMsg = new AgentMessage("assistant", finalContent);
                finalMsg.Thinking = string.IsNullOrWhiteSpace(finalSplit.Thinking)
                    ? null
                    : finalSplit.Thinking;
            ...
            if (hallucination.Detected)
            {
                Messages.Add(
                    new AgentMessage(
                        "assistant",
                        hallucination.CorrectionPrompt ?? "Let me try that again…"
                    )
                );
                NotifyStateChanged();
                await scrollAfterRender();
                ...
                if (hallucination.ArtifactId != null)
                {
                    ...
                    {
                        var followUp =
                            $"The artifact content is below. Summarise it accurately — do not fabricate anything.\n\n{showResult.Data}";
                            ...
                            && _gateway is ClusterChatGateway cg2 && cg2.IsEnabled)
                        {
                            var renderThrottle = System.Diagnostics.Stopwatch.StartNew();
                            await foreach (
                                var token in cg2.ChatStreamAsync(retryMessages, settings, ct)
                            )
                            {
                                StreamBuffer += token;
                                if (IsRepetitionLoopTail(StreamBuffer))
                        ...
                        if (!string.IsNullOrWhiteSpace(summarised))
                        {
                            ...
                            sumMsg.Thinking = string.IsNullOrWhiteSpace(sumSplit.Thinking)
                                ? null
                                : sumSplit.Thinking;
                            Messages.Add(sumMsg);
                        }
                    ...
                    else
                    {
                        Messages.Add(
                            new AgentMessage(
                                "assistant",
                                "Couldn't load the artifact content — it may be missing or corrupted."
                            )
                        );
                    }
                    Streaming = false;
                ...
                else
                {
                        ...
                        && _gateway is ClusterChatGateway cg3 && cg3.IsEnabled)
                    {
                        var renderThrottle = System.Diagnostics.Stopwatch.StartNew();
                        await foreach (
                            var token in cg3.ChatStreamAsync(retryMessages, settings, ct)
                        )
                        {
                            StreamBuffer += token;
                            if (IsRepetitionLoopTail(StreamBuffer))
                    ...
                    if (!string.IsNullOrWhiteSpace(retried))
                    {
                        if (DetectGenericHallucination(retried).Detected)
                            retried =
                                "The model got stuck repeating itself — please try asking again.";
                        var retryMsg = new AgentMessage("assistant", retried);
                        retryMsg.Thinking = string.IsNullOrWhiteSpace(retrySplit.Thinking)
                            ? null
                            : retrySplit.Thinking;
        ...
        catch (OperationCanceledException)
        {
            if (!Messages.Any(m => m.Role == "assistant" && m.Content.StartsWith("[stopped")))
                Messages.Add(
                    new AgentMessage("assistant", "[stopped — generation cancelled by user]")
                );
        }
        catch (Exception ex)
        {
    ...
    {
        var preamble = string.IsNullOrWhiteSpace(_preamble)
            ? AgentConfig.DefaultPreamble
            : _preamble;
        var toolDesc = string.IsNullOrWhiteSpace(_toolCatalog)
        ...
        for (var i = 0; i < Messages.Count; i++)
        {
            ...
            if (m.Role == "assistant")
            {
                ...
                if (
                    ...
                    && Messages[i + 1].Content.Contains("Tool Results")
                )
                {
                ...
                if (m.Role == "assistant")
                    content = CleanAssistantOutput(content);
            }
