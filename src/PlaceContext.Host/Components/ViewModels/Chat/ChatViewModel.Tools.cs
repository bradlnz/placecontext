...
public sealed partial class ChatViewModel
{
    ...
    {
        AddActiveAction(
            AgentToolNames.ListJobRuns,
            args != "" ? $"Runs for {args[..8]}..." : "Loading all runs..."
        );
        var jobId = Guid.TryParse(args, out var id) ? id : Guid.Empty;
        var runs =
            jobId != Guid.Empty ? await _svc.ListJobRunsAsync(jobId, ct) : new List<JobRunView>();
        var sb = new System.Text.StringBuilder();
    ...
    private async Task<ToolCallResult> ExecuteSearchAsync(string args, CancellationToken ct)
    {
        ...
        if (query.Length == 0)
            return ToolCallResult.Fail(
                $"Usage: {AgentToolNames.FormatCall(AgentToolNames.Search, "query")}"
            );

        AddActiveAction(
            AgentToolNames.Search,
            $"Searching \"{(query.Length > 30 ? query[..30] + "…" : query)}\"..."
        );
        try
        {
            ...
            if (matches.Count == 0)
            {
                ...
                return ToolCallResult.Ok(
                    "No matching run outputs found. Semantic search may be disabled (no embedding API key configured)."
                );
            }
            var sb = new System.Text.StringBuilder();
            ...
            foreach (var m in matches)
            {
                ...
                sb.Append(
                    $"- (score {m.Score:0.00}, run {m.JobRunId.ToString()[..8]}) {snippet}\n"
                );
            }
            CompleteActiveAction(AgentToolNames.Search, true);
        ...
        {
            CompleteActiveAction(AgentToolNames.Search, false);
            return ToolCallResult.Fail(ex.Message);
        }
    }
    ...
    private async Task<ToolCallResult> ExecuteGetArtifactsAsync(string args, CancellationToken ct)
    {
        ...
        var query = args.Trim();
        AddActiveAction(
            AgentToolNames.GetArtifacts,
            string.IsNullOrEmpty(query)
                ? ChatCopy.LoadingArtifacts()
                : ChatCopy.SearchingArtifacts(query)
        );
        try
        {
            ...
            else
            {
                ...
                if (terms.Count == 0)
                    artifacts = await _svc.ListProjectArtifactsAsync(
                        ProjectId.Value,
                        100,
                        null,
                        ct
                ...
                {
                    var broad = await _svc.ListProjectArtifactsAsync(
                        ProjectId.Value,
                        2000,
                        null,
                        ct
                    );
                    artifacts = ScoreAndFilterArtifacts(broad, terms);
                }
            }
            if (artifacts.Count == 0)
            {
                ...
                return ToolCallResult.Ok(
                    string.IsNullOrEmpty(query)
                        ? ChatCopy.NoArtifactsYet
                        : ChatCopy.ArtifactsMatchedNone(query)
                );
            }
            ...
            foreach (var a in artifacts)
                sb.Append(
                    $"- {a.Title} | {a.Kind} | {Helpers.FormatHelper.Bytes(a.SizeBytes)} | {a.CreatedAt:yyyy-MM-dd HH:mm} | id:{a.Id} | /runs/{a.RunId}/artifacts/{a.Id}\n"
                );
            sb.Append(
        ...
        {
            CompleteActiveAction(AgentToolNames.GetArtifacts, false);
            return ToolCallResult.Fail(ex.Message);
        }
    }
    ...
    private static IReadOnlyList<ArtifactFileView> ScoreAndFilterArtifacts(
        IReadOnlyList<ArtifactFileView> artifacts,
        ...
        artifacts
            .Select(a =>
            {
                var haystack = $"{a.Title} {a.Kind}".ToLowerInvariant();
                return (Artifact: a, Score: terms.Count(t => haystack.Contains(t)));
            })
            .Where(x => x.Score > 0)
            .OrderByDescending(x => x.Score)
            .ThenByDescending(x => x.Artifact.CreatedAt)
            .Take(25)
            .Select(x => x.Artifact)
            .ToList();
    ...
    internal async Task<ToolCallResult> ExecuteShowArtifactAsync(string args, CancellationToken ct)
    {
        ...
        if (!Guid.TryParse(idStr, out var artifactId))
            return ToolCallResult.Fail(
                $"Usage: {AgentToolNames.FormatCall(AgentToolNames.ShowArtifact, "artifactId")}"
            );

        AddActiveAction(
            AgentToolNames.ShowArtifact,
            ChatCopy.LoadingArtifact(artifactId.ToString()[..8])
        );
        try
        {
            ...
            if (link is null)
            {
                CompleteActiveAction(AgentToolNames.ShowArtifact, false);
                return ToolCallResult.Fail(ChatCopy.ArtifactNotFound);
            }
            ...
            if ((isTextLike || isDocument) && _objectStore.IsEnabled)
            {
                try
                {
                    ...
                    if (obj is not null)
                    {
                        ...
                        int read,
                            total = 0;
                        var maxBytes = isDocument
                            ? MaxArtifactDocumentBytes
                        ...
                        if (isDocument)
                        {
                            var rawText = _docExtractor.ExtractText(bytes, link.Title);
                            if (!string.IsNullOrWhiteSpace(rawText))
                            {
                                if (rawText.Length > MaxArtifactExtractedTextLength)
                                {
                                    extractedText = rawText[..MaxArtifactExtractedTextLength];
    ...
    private static bool IsTextArtifactContentType(string ct) =>
        ct.StartsWith("text/")
        ...
        || ct.Contains("svg", StringComparison.OrdinalIgnoreCase);

    private static bool IsDocumentContentType(string ct) =>
        ct == "application/pdf"
    ...
    private async Task<ToolCallResult> ExecuteRenderGraphAsync(string args, CancellationToken ct)
    {
        ...
        if (!tableValid)
        {
            ...
            var clarify = await AskClarificationAsync(
                ...
                {
                    ToolName = AgentToolNames.RenderGraph,
                    Args = args,
                    Question =
                        $"Table '{tableName}' not found. Which table would you like to chart?",
                    MultiSelect = false,
                    Options = tables
                        .Where(t => t.RowEstimate > 0)
                        .Select(t => new ClarificationOption
                        {
                            Id = t.Name,
                            Label = t.Name,
        ...
        if (columns.Count == 0)
        {
            ...
            {
                if (
                    numericProbe.Rows.Any(r =>
                        i < r.Count && double.TryParse(r[i]?.ToString(), out _)
                    )
                )
                    numericCols.Add(numericProbe.Columns[i]);
            }
            var clarify = await AskClarificationAsync(
                new ClarificationRequest
                {
                    ToolName = AgentToolNames.RenderGraph,
                    Args = args,
                    Question = $"Which column(s) in '{tableName}' should be charted?",
                    MultiSelect = true,
                    Options = numericCols
                        .Select(c => new ClarificationOption { Id = c, Label = c })
                        .ToList(),
                }
            );
            if (!clarify.Confirmed || clarify.SelectedIds.Count == 0)
        ...
        foreach (var col in columns)
        {
            var colIndex = result
                .Columns.ToList()
                .FindIndex(c => c.Equals(col, StringComparison.OrdinalIgnoreCase));
            if (colIndex >= 0)
        ...
        foreach (var row in result.Rows)
        {
            ...
            for (var s = 0; s < seriesList.Count; s++)
            {
                var colIndex = result
                    .Columns.ToList()
                    .FindIndex(c => c.Equals(columns[s], StringComparison.OrdinalIgnoreCase));
                var valStr =
                    colIndex >= 0 && colIndex < row.Count ? row[colIndex]?.ToString() ?? "0" : "0";
                seriesList[s].Values.Add(double.TryParse(valStr, out var val) ? val : 0);
            }
        }
        var graphData = new
        {
            type = chartType,
            title = $"{tableName}"
                + (columns.Count > 1 ? $" — {string.Join(", ", columns)}" : $" — {columns[0]}"),
            labels = labels.Take(24).ToList(),
            series = seriesList
                .Select(s => new { name = s.Name, values = s.Values.Take(24).ToList() })
                .ToList(),
        };
    ...
    private async Task<ToolCallResult> ExecuteQueryGraphAsync(CancellationToken ct)
    {
        ...
        try
        {
            ...
            var byKind = graph
                .Nodes.GroupBy(n => n.Kind ?? "unknown")
            ...
            if (hubs.Count > 0)
            {
                ...
                foreach (var n in hubs)
                    sb.AppendLine(
                        $"- **{n.Label}** ({n.Degree} connections){(n.IsGod ? " ⭐" : "")}"
                    );
                sb.AppendLine();
            }
            GraphNodes.Clear();
            GraphNodes.AddRange(graph.Nodes.OrderByDescending(n => n.Degree).Take(50));
            ...
            if (graph.Links.Count > 0)
            {
                ...
                foreach (var edge in graph.Links)
                    sb.AppendLine(
                        $"- {nodeLabels.GetValueOrDefault(edge.Source) ?? "?"} → {nodeLabels.GetValueOrDefault(edge.Target) ?? "?"} ({edge.Confidence})"
                    );
            }
            CompleteActiveAction(AgentToolNames.QueryGraph, true);
            AddToolHistory(AgentToolNames.QueryGraph, true, $"{graph.NodeCount} nodes");
            return ToolCallResult.Ok(sb.ToString());
        }
        catch (Exception ex)
        {
            CompleteActiveAction(AgentToolNames.QueryGraph, false);
            AddToolHistory(AgentToolNames.QueryGraph, false, ex.Message);
    ...
    private async Task<ToolCallResult> ExecuteRenderMapAsync(string args, CancellationToken ct)
    {
        if (string.IsNullOrWhiteSpace(args))
            return ToolCallResult.Fail(ChatCopy.RenderMapUsage);
        try
        {
            System.Text.Json.JsonDocument.Parse(args.Trim());
        }
        catch
        {
            return ToolCallResult.Fail(ChatCopy.InvalidRenderMapJson);
        }
        AddActiveAction(AgentToolNames.RenderMap, ChatCopy.RenderingMap);
        await Task.Delay(10, ct);
        CompleteActiveAction(AgentToolNames.RenderMap, true);
        AddToolHistory(AgentToolNames.RenderMap, true, ChatCopy.MapRendered);
    ...
    private async Task<ToolCallResult> ExecuteScheduleJobAsync(string args, CancellationToken ct)
    {
        var parts = args.Split('|');
        if (parts.Length < 3)
            return ToolCallResult.Fail("Usage: schedule_job|jobId|name|cron");
        var jobId = Guid.TryParse(parts[0].Trim(), out var id) ? id : Guid.Empty;
        if (jobId == Guid.Empty)
            return ToolCallResult.Fail("Invalid jobId");
        AddActiveAction(
            AgentToolNames.ScheduleJob,
            $"Creating schedule for job {jobId.ToString()[..8]}..."
        );
        var trigger = await _svc.CreateTriggerAsync(
            new CreateTriggerCommand(jobId, parts[1].Trim(), "Schedule", parts[2].Trim(), null),
            ct
        );
        CompleteActiveAction(AgentToolNames.ScheduleJob, true);
        AddToolHistory(
            AgentToolNames.ScheduleJob,
            true,
            $"Next: {trigger.NextRunAt?.ToString("HH:mm") ?? "—"}"
        );
        return ToolCallResult.Ok(
            $"Schedule created: {trigger.Name} (id: {trigger.Id})\nCron: {trigger.CronExpression}\nNext run: {trigger.NextRunAt?.ToString("yyyy-MM-dd HH:mm") ?? "—"}\nEnabled: {trigger.Enabled}"
        );
    }
    ...
    private async Task<ToolCallResult> ExecuteListSchedulesAsync(string args, CancellationToken ct)
    {
        ...
        foreach (var t in triggers)
        {
            ...
            if (t.Kind == "Schedule")
                sb.Append(
                    $"  Cron: {t.CronExpression} | Next: {t.NextRunAt?.ToString("yyyy-MM-dd HH:mm") ?? "—"}\n"
                );
            sb.Append(
                $"  Enabled: {t.Enabled} | Last fired: {t.LastFiredAt?.ToString("yyyy-MM-dd HH:mm") ?? "never"}\n"
            );
        }
        return ToolCallResult.Ok(sb.ToString());
    }
    ...
    private async Task<ToolCallResult> ExecuteToggleScheduleAsync(string args, CancellationToken ct)
    {
        var parts = args.Split('|');
        if (parts.Length < 2)
            return ToolCallResult.Fail("Usage: toggle_schedule|triggerId|true|false");
        var triggerId = Guid.TryParse(parts[0].Trim(), out var id) ? id : Guid.Empty;
        if (triggerId == Guid.Empty)
            return ToolCallResult.Fail("Invalid triggerId");
        var enabled = parts[1].Trim().ToLower() == "true";
        AddActiveAction(
            AgentToolNames.ToggleSchedule,
            $"Toggling schedule {triggerId.ToString()[..8]}..."
        );
        var trigger = await _svc.SetTriggerEnabledAsync(triggerId, enabled, ct);
        CompleteActiveAction(AgentToolNames.ToggleSchedule, true);
        AddToolHistory(AgentToolNames.ToggleSchedule, true, enabled ? "enabled" : "disabled");
        return ToolCallResult.Ok(
            $"Schedule '{trigger.Name}' {(enabled ? "enabled" : "disabled")}.\nNext run: {trigger.NextRunAt?.ToString("yyyy-MM-dd HH:mm") ?? "—"}"
        );
    }
    ...
    private async Task<ToolCallResult> ExecuteRunJobAsync(string args, CancellationToken ct)
    {
        ...
        AddToolHistory(AgentToolNames.RunJob, true, run.Status);
        return ToolCallResult.Ok(
            $"Job run started: {run.Id}\nStatus: {run.Status}\nStarted: {run.StartedAt:yyyy-MM-dd HH:mm}"
        );
    }
    ...
    private async Task<ToolCallResult> ExecuteRunJobChainAsync(string args, CancellationToken ct)
    {
        ...
        var run = await _svc.RunJobChainAsync(
            chainId,
            payload,
            chainRunId: null,
        ...
        {
            var output =
                run.FinalOutput.Length > 2000 ? run.FinalOutput[..2000] + "…" : run.FinalOutput;
            sb.Append($"\nFinal output:\n{output}");
        }
    ...
    private async Task<(Guid ChainId, string? Error)> ResolveChainIdAsync(
        string key,
        CancellationToken ct
    )
    {
    ...
    private async Task<ToolCallResult> ExecuteCallMcpAsync(string args, CancellationToken ct)
    {
        ...
        if (string.IsNullOrEmpty(serverName))
        {
            ...
            foreach (var c in mcps)
                sb.AppendLine(
                    $"- {c.Name} ({c.Transport}) - {(c.Enabled ? "enabled" : "disabled")}"
                );
        ...
        if (connection == null)
        {
            CompleteActiveAction(AgentToolNames.CallMcp, false);
            return ToolCallResult.Fail(
                ChatCopy.McpServerNotFound(
                    serverName,
        ...
        try
        {
            ...
            CompleteActiveAction(AgentToolNames.CallMcp, result.Success);
            AddToolHistory(
                AgentToolNames.CallMcp,
                result.Success,
                result.Success ? "ok" : result.Error ?? "error"
            );
            return result.Success
                ? ToolCallResult.Ok(result.Content ?? "Tool executed successfully")
                : ToolCallResult.Fail($"MCP tool error: {result.Error}");
        }
        catch (Exception ex)
        {
            CompleteActiveAction(AgentToolNames.CallMcp, false);
            return ToolCallResult.Fail(ChatCopy.McpCallFailed(ex.Message));
        }
    }
    ...
    private async Task<ToolCallResult> ExecuteListMcpToolsAsync(string args, CancellationToken ct)
    {
        ...
        if (string.IsNullOrEmpty(serverName))
        {
            ...
            foreach (var conn in connections)
                sb.AppendLine(
                    $"- {conn.Name} ({conn.Transport}) - {(conn.Enabled ? "enabled" : "disabled")}"
                );
