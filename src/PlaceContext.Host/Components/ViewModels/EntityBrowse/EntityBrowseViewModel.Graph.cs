...
public sealed partial class EntityBrowseViewModel
{
    ...
    private async Task LoadLinkedArtifactsAsync(IReadOnlyList<string?> row)
    {
        ...
        {
            var li = cols.FindIndex(c =>
                string.Equals(c, label, StringComparison.OrdinalIgnoreCase)
            );
            if (li >= 0 && li < row.Count && row[li] is { Length: > 2 } lv)
                keys.Add(lv);
        }
        foreach (var rel in Entity.Relations)
        {
            var ci = cols.FindIndex(c =>
                string.Equals(c, rel.Column, StringComparison.OrdinalIgnoreCase)
            );
            if (ci >= 0 && ci < row.Count && row[ci] is { Length: > 2 } cv)
                keys.Add(cv);
        }
        ...
        foreach (var key in keys.Distinct().Take(3))
        {
            try
            {
                foreach (var id in await _svc.ListTaggedRunsAsync(Entity.Id, key))
                    runIds.Add(id);
            }
            catch { }
        }
        ...
        foreach (var runId in runIds.Take(8))
        {
            ...
            {
                RunArtifacts[runId] = new List<RunArtifactLinkView>();
            }
        }
        Artifacts.AddRange(
            collected
    ...
    public async Task BuildInsightsAsync()
    {
        ...
        try
        {
            ...
            {
                var r = await _svc.ExecuteProjectDataAsync(
                    ProjectId,
            ...
            var keyCols = Entity
                .Relations.Select(rel => rel.Column)
                .Concat(
                    columns
                        .Where(c => c.Type.Contains("text") || c.Type.Contains("char"))
                        .Select(c => c.Name)
                )
                .Where(c =>
                    !string.Equals(c, Entity.LabelColumn, StringComparison.OrdinalIgnoreCase)
                )
    ...
    {
        BrainGraph = null;
        if (Entity is null || Rows is null || Rows.Rows.Count == 0)
            return;
        try
        {
            ...
            var sourceRows = FocusKey is { } focus
                ? Rows
                    .Rows.Where(r =>
                        r.Any(v => string.Equals(v, focus, StringComparison.OrdinalIgnoreCase))
                    )
                    .ToList()
                : Rows.Rows;
            var labelIdx = LabelCol() is { } lc
                ? Math.Max(
                    0,
                    cols.FindIndex(c => string.Equals(c, lc, StringComparison.OrdinalIgnoreCase))
                )
                : 0;
            var records = sourceRows
                .Take(maxRecords)
                .Select(r => labelIdx < r.Count ? r[labelIdx] ?? "—" : "—")
                .ToList();
            ...
            foreach (var rel in Entity.Relations.Take(2))
            {
                ...
                try
                {
                    var join = await _svc.ExecuteProjectDataAsync(
                        ProjectId,
                    ...
                    {
                        if (row.Count < 2 || row[0] is not { } k || row[1] is not { } lbl)
                            continue;
                        if (!keyToRecord.TryGetValue(k, out var recIdx))
                            continue;
                        var ni = relatedNodes.FindIndex(n =>
                            n.Entity == target.Name && n.Label == lbl
                        );
                        if (ni < 0)
                        {
                            relatedNodes.Add((target.Name, lbl));
                            ni = relatedNodes.Count - 1;
                        }
                        if (relatedNodes.Count > maxRecords)
                            break;
                        if (!relEdges.Contains((recIdx, ni)))
                            relEdges.Add((recIdx, ni));
                    }
                }
                catch { }
            }
            ...
            {
                if (!keyToRecord.TryGetValue(pair.Key, out var recIdx))
                    continue;
                ...
                if (!runEdges.Contains((recIdx, ri)))
            ...
            for (var ri = 0; ri < runNodes.Count; ri++)
            {
                try
                {
                    foreach (var a in await _svc.ListRunArtifactsAsync(runNodes[ri]))
                        allArts.Add((ri, a));
                }
                catch { }
            }
            var artNodes = allArts
                ...
                .Select(g =>
                    (g.OrderByDescending(x => x.Art.CreatedAt).First(), Versions: g.Count())
                )
    ...
    )
    {
        ...
        for (var i = 0; i < related.Count; i++)
        {
            nodes.Add(
                new GraphNodeView(
                    ...
                    0,
                    false,
                    Content: $"{related[i].Entity} record (related)",
                    Kind: "human"
                )
            );
            NodeUrls[$"rel:{i}"] =
                $"/project/{ProjectId}/entity/{Uri.EscapeDataString(related[i].Entity)}?record={Uri.EscapeDataString(related[i].Label)}";
        }
        ...
        foreach (var (rec, rel) in relEdges)
            Link($"rec:{rec}", $"rel:{rel}");
        foreach (var (rec, run) in runEdges)
            Link($"rec:{rec}", $"run:{run}");
        for (var i = 0; i < arts.Count; i++)
            Link($"run:{arts[i].Run}", $"art:{i}");
