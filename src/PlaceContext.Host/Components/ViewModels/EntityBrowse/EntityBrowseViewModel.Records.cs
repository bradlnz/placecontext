...
public sealed partial class EntityBrowseViewModel
{
    ...
    public async Task LoadRecordsPageAsync()
    {
        ...
        try
        {
            Page = await _svc.QueryProjectTablePageAsync(
                ProjectId,
                Entity.TableName,
                Search,
                PageNum,
    ...
    public string? LabelCol()
    {
        ...
        if (
            Entity?.LabelColumn is { Length: > 0 } configured
            && cols.FirstOrDefault(c =>
                string.Equals(c, configured, StringComparison.OrdinalIgnoreCase)
            )
                is { } exact
        )
            return exact;
        for (var i = 0; i < cols.Count; i++)
        {
            ...
            if (
                ...
                || string.Equals(c, "ingested_at", StringComparison.OrdinalIgnoreCase)
            )
                continue;
            var v = Rows.Rows.FirstOrDefault() is { } first && i < first.Count ? first[i] : null;
            if (v is null || double.TryParse(v, out _))
                continue;
            return c;
        }
    ...
    public static string ComputeRowKey(
        string table,
    ...
    )
    {
        ...
        var entity = entities.FirstOrDefault(e =>
            string.Equals(e.TableName, table, StringComparison.OrdinalIgnoreCase)
        );
        if (entity?.LabelColumn is { Length: > 0 } label)
        {
            var li = colNamesList.FindIndex(c =>
                string.Equals(c, label, StringComparison.OrdinalIgnoreCase)
            );
            if (li >= 0 && li < row.Count && !string.IsNullOrEmpty(row[li]))
                return row[li]!;
        }
        var values = columns
            .Where(c =>
            ...
            .Select(name =>
            {
                var i = colNamesList.FindIndex(n =>
    ...
    public async Task OpenRecordAsync(IReadOnlyList<string?> row)
    {
        ...
        foreach (var rel in Entity.Relations)
        {
            ...
            var colIdx = Rows
                .Columns.ToList()
                .FindIndex(c => string.Equals(c, rel.Column, StringComparison.OrdinalIgnoreCase));
            if (target is null || colIdx < 0 || colIdx >= row.Count || row[colIdx] is not { } value)
                continue;
            try
            {
                var related = await _svc.ExecuteProjectDataAsync(
                    ProjectId,
    ...
    public async Task StartEditAsync()
    {
        ...
        EditError = null;
        var columnInfos = (
            await _svc.ListProjectTableColumnsAsync(ProjectId, Entity.TableName)
        ).ToDictionary(c => c.Name, c => c.Type, StringComparer.Ordinal);
        FormColumns = (Rows?.Columns ?? Array.Empty<string>())
            .Select(c => new FormColumn(c, columnInfos.GetValueOrDefault(c, DataColumnTypes.Text)))
            .ToList();
        ...
        if (label is not null && KeySnapshot.ContainsKey(label))
            KeySnapshot = new Dictionary<string, string?>(StringComparer.Ordinal)
            {
                [label] = KeySnapshot[label],
            };
        else
        {
            var kept = KeySnapshot
    ...
    public async Task StartCreateAsync()
    {
        ...
        if (FormColumns.Count == 0 && Page is { Columns.Count: > 0 })
            FormColumns = Page
                .Columns.Select(c => new FormColumn(c, DataColumnTypes.Text))
                .ToList();
        FormValues = FormColumns.ToDictionary(
            c => c.Name,
            _ => (string?)null,
    ...
    {
        if (Entity is null || FormValues.Count == 0)
            return;
        Saving = true;
        EditError = null;
        WarnMessage = null;
        try
        {
            if (Creating)
            {
                var created = await _svc.CreateEntityRecordAsync(
                    ProjectId,
                    Entity.TableName,
                    FormValues
                );
                WarnMessage =
                    created.DuplicateWarnings.Count > 0
                        ? "Created — possible duplicate(s): "
                            + string.Join("; ", created.DuplicateWarnings)
                        : null;
            }
            else
            {
                await _svc.UpdateEntityRecordAsync(
                    ProjectId,
                    Entity.TableName,
                    KeySnapshot,
                    FormValues
    ...
    {
        if (Entity is null || Open is null || Rows is null)
            return;
    ...
    public string GraphSvg()
    {
        ...
        var relNodes = Related
            .SelectMany(r =>
                r.Rows.Rows.Take(3)
                    .Select(row => (r.Relation.TargetEntity, Label: row.FirstOrDefault() ?? "—"))
            )
            .Take(6)
            .ToList();
        ...
        for (var i = 0; i < relNodes.Count; i++)
        {
            ...
            sb.Append(
                FormattableString.Invariant(
                    $"<line x1='150' y1='{y}' x2='216' y2='{cy}' stroke='var(--border-2)' stroke-width='1.2'/>"
                )
            );
            sb.Append(
                FormattableString.Invariant(
                    $"<rect x='8' y='{y - 14}' width='142' height='28' rx='7' fill='var(--human-bg)' stroke='var(--human)'/>"
                )
            );
            sb.Append(
                FormattableString.Invariant(
                    $"<text x='16' y='{y - 1}' font-size='9.5' fill='var(--human)' font-family='JetBrains Mono, monospace'>{E(relNodes[i].TargetEntity)}</text>"
                )
            );
            sb.Append(
                FormattableString.Invariant(
                    $"<text x='16' y='{y + 10}' font-size='9' fill='var(--text-2)'>{E(FormatHelper.Trunc(relNodes[i].Label, 22))}</text>"
                )
            );
        }
        ...
        sb.Append(
            FormattableString.Invariant(
                $"<rect x='216' y='{cy - 17}' width='120' height='34' rx='8' fill='var(--brand-bg)' stroke='var(--brand-2)' stroke-width='1.4'/>"
            )
        );
        sb.Append(
            FormattableString.Invariant(
                $"<text x='276' y='{cy - 2}' font-size='10' font-weight='600' fill='var(--text)' text-anchor='middle'>{E(FormatHelper.Trunc(Open is null ? "" : RecordLabel(Open), 18))}</text>"
            )
        );
        sb.Append(
            FormattableString.Invariant(
                $"<text x='276' y='{cy + 10}' font-size='8.5' fill='var(--text-3)' text-anchor='middle'>{E(Entity?.Name ?? "")} record</text>"
            )
        );
        ...
        foreach (var runId in LinkedRuns)
        {
            ...
            sb.Append(
                FormattableString.Invariant(
                    $"<line x1='336' y1='{cy}' x2='376' y2='{y}' stroke='var(--border-2)' stroke-width='1.2' stroke-dasharray='4 3'/>"
                )
            );
            sb.Append($"<a href='/observability?run={runId}'>");
            sb.Append(
                FormattableString.Invariant(
                    $"<rect x='376' y='{y - 13}' width='132' height='26' rx='7' fill='var(--card-2)' stroke='var(--border-2)' style='cursor:pointer'/>"
                )
            );
            sb.Append(
                FormattableString.Invariant(
                    $"<text x='384' y='{y + 3}' font-size='9' fill='var(--text-2)' font-family='JetBrains Mono, monospace' style='cursor:pointer'>run {runId.ToString("N")[..8]} ↗</text>"
                )
            );
            sb.Append("</a>");
            for (var k = 0; k < arts.Count; k++)
            {
                ...
                sb.Append(
                    FormattableString.Invariant(
                        $"<line x1='442' y1='{y + 13}' x2='442' y2='{ay - 11}' stroke='var(--border-2)'/>"
                    )
                );
                sb.Append($"<a href='/artifacts?artifact={arts[k].Id}'>");
                sb.Append(
                    FormattableString.Invariant(
                        $"<rect x='396' y='{ay - 11}' width='112' height='22' rx='6' fill='var(--good-bg)' stroke='var(--good)' style='cursor:pointer'/>"
                    )
                );
                sb.Append(
                    FormattableString.Invariant(
                        $"<text x='403' y='{ay + 3}' font-size='8.5' fill='var(--good)' font-family='JetBrains Mono, monospace' style='cursor:pointer'>{E(FormatHelper.Trunc(arts[k].Title, 17))} ↗</text>"
                    )
                );
                sb.Append("</a>");
            }
