...
public sealed partial class DataMapViewModel
{
    ...
    public async Task OnCanvasUpAsync()
    {
        ...
        if (DragKey is not null)
        {
            ...
            {
                if (Mappings?.FirstOrDefault(m => m.JobId == jobId) is { } existing)
                    OpenEditor(existing);
                else if (Jobs?.FirstOrDefault(j => j.Id == jobId) is { } job)
    ...
    public string EdgePath(DataMappingView m)
    {
        ...
        var x1 = from.X + 190 + PanX;
        var y1 = from.Y + 26 + PanY;
        var x2 = to.X + PanX;
        var y2 = to.Y + 26 + PanY;
    ...
    {
        if (ConnectJobId is null)
            return "";
        var from = GetPos(ConnectJobId);
        var x1 = from.X + 190 + PanX;
        var y1 = from.Y + 22 + PanY;
        return FormattableString.Invariant(
            $"M {x1} {y1} L {ConnectEnd.X + PanX} {ConnectEnd.Y + PanY}"
        );
    }
}
