...
public sealed partial class ProjectDataViewModel
{
    ...
    public static string InferType(IEnumerable<string?> values)
    {
        ...
        if (sample.Count == 0) return DataColumnTypes.Text;
        bool All(Func<string, bool> f) => sample.All(f);
        if (All(v => v is "true" or "false" or "TRUE" or "FALSE" or "True" or "False")) return DataColumnTypes.Boolean;
        if (All(v => long.TryParse(v, System.Globalization.NumberStyles.Integer, ci, out _))) return DataColumnTypes.Bigint;
        if (All(v => decimal.TryParse(v, System.Globalization.NumberStyles.Number, ci, out _))) return DataColumnTypes.Numeric;
        if (All(v => Guid.TryParse(v, out _))) return DataColumnTypes.Uuid;
        if (All(v => DateTime.TryParse(v, ci, System.Globalization.DateTimeStyles.None, out _))) return DataColumnTypes.Timestamptz;
