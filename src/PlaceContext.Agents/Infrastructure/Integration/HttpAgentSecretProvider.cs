...
public sealed class HttpAgentSecretProvider(
    IHttpClientFactory httpClientFactory,
    IConfiguration configuration) : IAgentSecretProvider
{
    ...
    {
        var origin = configuration["PlaceContext:Agents:Vault:BaseAddress"]
            ?? configuration["PlaceContext:Microservices:Destinations:Vault"]
            ?? throw new InvalidOperationException("Configure the Vault service destination for Agents.");
