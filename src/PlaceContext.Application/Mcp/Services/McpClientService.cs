...
public sealed class McpClientService : IMcpClientService
{
    ...
    private HttpClient CreateClient(McpConnection connection)
    {
        ...
        if (!string.IsNullOrEmpty(connection.AuthToken))
        {
            if (connection.AuthType == McpAuthType.Bearer || connection.AuthType == McpAuthType.OAuth)
            {
        ...
        if (!string.IsNullOrEmpty(connection.AuthHeader) && !string.IsNullOrEmpty(connection.AuthToken))
        {
