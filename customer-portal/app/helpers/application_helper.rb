module ApplicationHelper
  ICONS = {
    ...
    artifacts: '<svg viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8l-6-6Z" stroke="currentColor" stroke-width="2" stroke-linejoin="round"/><path d="M14 2v6h6M8 13h8M8 17h5" stroke="currentColor" stroke-width="2" stroke-linecap="round"/></svg>',
    users: '<svg viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true"><path d="M12 12c2.21 0 4-1.79 4-4s-1.79-4-4-4-4 1.79-4 4 1.79 4 4 4z" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/><path d="M4 20c0-2.21 3.13-4 8-4s8 1.79 8 4" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/><path d="M4 4h16M4 20h16" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg>',
  ...
  def portal_brand_name
    env_name = ENV["PORTAL_BRAND_NAME"].to_s.strip
    env_name.presence || "PlaceContext"
  end

  def portal_brand_logo_url
    logo_url = ENV["PORTAL_BRAND_LOGO_URL"].to_s.strip
    logo_url.presence
  end
  ...
  def artifact_kind(content_type)
    value = content_type.to_s.downcase
    return ["PDF", "artifact-type-pdf"] if value.include?("pdf")
    return ["IMG", "artifact-type-image"] if value.start_with?("image/")
    return ["CSV", "artifact-type-sheet"] if value.include?("csv") || value.include?("spreadsheet")
    return ["JSON", "artifact-type-code"] if value.include?("json")
    return ["DOC", "artifact-type-doc"] if value.include?("document") || value.include?("word")

    ["FILE", "artifact-type-default"]
  end
end
