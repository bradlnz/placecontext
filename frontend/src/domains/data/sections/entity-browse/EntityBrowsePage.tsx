...
function rowValues(model: EntityBrowseModel, row: Array<string | null>): RecordValues {
...
function rowKeys(model: EntityBrowseModel, values: RecordValues): RecordValues {
  ...
  if (primaryKeys.length > 0) {
    return Object.fromEntries(primaryKeys.map((column) => [column.name, values[column.name] ?? null]))
  }
...
export function EntityBrowsePage() {
  ...
  async function openRecord(row: Array<string | null>): Promise<void> {
            ...
            <tbody>
              {data.page.rows.length === 0 ? (
                ...
                data.page.rows.map((row, rowIndex) => (
                  <tr key={`${String(rowIndex)}:${row.join(':')}`} onClick={() => void openRecord(row)}>
              ...
              <div>
                {editValues !== null ? null : (
                  <>
                    ...
                    <button className="dcbtn danger" onClick={() => void removeRecord()} type="button">
                      Delete
                    </button>
                  </>
                )}
              ...
              {editValues === null && openValues !== null ? (
                ...
                >
                    ...
                    <label key={column.name}>
                      ...
                      {column.type === 'boolean' ? (
                        ...
                        <input
                          className="dcinput"
                          onChange={(event) =>
                            setEditValues({ ...editValues, [column.name]: event.target.value || null })
                          }
                  ...
                  <footer>
                    ...
                    <button className="dcbtn primary" disabled={writeMutation.isPending} type="submit">
