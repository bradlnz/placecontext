...
describe('ConnectionsPage', () => {
  it('shows project connection status and validates the database form', async () => {
    ...
    queryClient.setQueryData(connectionsQueryOptions.queryKey, {
      projects: [{ id: '158fdb23-5c46-4777-b0bb-d78ff91b8754', name: 'Atlas', hasExternalDatabase: false, hasExternalIndex: true }],
      sslModes: ['Prefer', 'Require'],
    })
    render(<QueryClientProvider client={queryClient}><AppEventBusProvider><ConnectionsPage /></AppEventBusProvider></QueryClientProvider>)
    expect(screen.getByText(/Atlas — using the cluster database · external search index configured/)).toBeVisible()
