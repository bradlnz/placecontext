...
export function CommunicationProviderForm({
  busy,
  draft,
  editing,
  projects,
  ...
  const kinds =
    draft.channel === 'sms' ? (['twilio'] as const) : (['postmark', 'sendgrid'] as const)
    ...
    <section className="dccard communication-form">
      ...
      <div className="settings-field-grid two-columns">
        <label className="dcfield">
          ...
          <select
            onChange={(event) =>
              void changeChannel(event.target.value as CommunicationProviderDraft['channel'])
            }
            value={draft.channel}
          >
        ...
        <label className="dcfield">
          ...
          <select
            onChange={(event) =>
              void changeKind(event.target.value as CommunicationProviderDraft['kind'])
            }
            value={draft.kind}
          >
            {kinds.map((kind) => (
              <option key={kind} value={kind}>
                {kind === 'sendgrid' ? 'SendGrid' : kind === 'twilio' ? 'Twilio' : 'Postmark'}
              </option>
            ))}
          </select>
        </label>
        <label className="dcfield">
          ...
          <input
            ...
            placeholder="Transactional email"
            value={draft.name}
          />
        </label>
        <label className="dcfield">
          ...
          <select
            onChange={(event) =>
              void changeAuthType(event.target.value as CommunicationProviderDraft['authType'])
            }
      ...
      {draft.authType === 'header' ? (
        <label className="dcfield">
          ...
          <input
            ...
            onChange={(event) => void patch({ authHeaderName: event.target.value })}
            placeholder="X-Api-Key"
            value={draft.authHeaderName ?? ''}
          />
          <small>
            The header that carries the API key. Postmark uses <code>X-Postmark-Server-Token</code>.
          </small>
        </label>
      ) : null}
      {draft.authType === 'basic' ? (
        <div className="settings-message">
          Basic auth takes its username from the Account SID field below; the Vault secret is the
          auth token used as the password.
        </div>
      ) : null}
      {usesSecret ? (
        <div className="settings-field-grid two-columns">
          <label className="dcfield">
            ...
            <select
              ...
              onChange={(event) => void onProjectChange(event.target.value || null)}
              value={draft.vaultProjectId ?? ''}
            >
          ...
          <label className="dcfield">
            ...
            <select
              ...
              disabled={draft.vaultProjectId === null}
              onChange={(event) => void patch({ apiKeySecretName: event.target.value || null })}
            ...
            {draft.vaultProjectId !== null && secrets.length === 0 ? (
              <small className="warning">No Vault secrets exist in this project yet.</small>
            ) : null}
          </label>
        </div>
      ) : null}
      {draft.kind === 'twilio' ? (
        <div className="settings-field-grid two-columns">
          <label className="dcfield">
            ...
            <input
              ...
              onChange={(event) => void patch({ accountSid: event.target.value })}
              placeholder="AC…"
          ...
          <label className="dcfield">
            ...
            <input
              onChange={(event) => void patch({ fromNumber: event.target.value })}
        ...
        <div className="settings-field-grid two-columns">
          <label className="dcfield">
            ...
            <input
              aria-label="Verified sender email"
              onChange={(event) => void patch({ fromEmail: event.target.value })}
          ...
          <label className="dcfield">
            ...
            <input
              onChange={(event) => void patch({ fromName: event.target.value })}
          ...
          {draft.kind === 'postmark' ? (
            <label className="dcfield">
              ...
              <input
                ...
                onChange={(event) => void patch({ messageStream: event.target.value })}
                placeholder="outbound"
                value={draft.messageStream}
              />
              <small>
                Use <code>outbound</code> unless you created a dedicated transactional stream.
              </small>
            </label>
          ) : null}
        </div>
      ...
      <label className="dcfield">
        ...
        <input
          onChange={(event) => void patch({ endpoint: event.target.value })}
          placeholder="Leave empty for the provider default"
          value={draft.endpoint}
        />
      </label>
      <div className="settings-actions">
        <button className="dcbtn" onClick={() => void onCancel()} type="button">
          Cancel
        </button>
        <button
