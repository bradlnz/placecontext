...
import {
  ...
  fetchCommunicationSecrets,
  sendCommunicationProviderTest,
  setCommunicationProviderTwoFactor,
  setDefaultCommunicationProvider,
  updateCommunicationProvider,
} from '../../api/communications-api'
import type {
  CommunicationProvider,
  CommunicationProviderDraft,
...
function providerDraft(provider: CommunicationProvider): CommunicationProviderDraft {
  return {
    ...
    messageStream: readSetting(provider.settingsJson, 'messageStream') || 'outbound',
  ...
  const settings =
    draft.kind === 'twilio'
      ...
      : {
          ...
          ...(draft.kind === 'postmark'
            ? { messageStream: draft.messageStream.trim() || 'outbound' }
            : {}),
          ...(draft.endpoint.trim() === '' ? {} : { endpoint: draft.endpoint.trim() }),
        }
  return {
    ...
    authHeaderName: draft.authType === 'header' ? draft.authHeaderName : null,
    vaultProjectId: draft.authType === 'none' ? null : draft.vaultProjectId,
    apiKeySecretName: draft.authType === 'none' ? null : draft.apiKeySecretName,
...
export function CommunicationsPage() {
  ...
  useEffect(() => {
    ...
    const unsubscribeAction = eventBus.subscribe(
      'settings.communication-action-requested',
      async (action) => {
        setMessage(null)
        setBusy(true)
        try {
          if (action.kind === 'delete') {
            await deleteCommunicationProvider(action.providerId, AbortSignal.timeout(30_000))
            await changed(action.providerId, 'Provider deleted.')
            return
          }
          if (action.kind === 'default') {
            await setDefaultCommunicationProvider(action.providerId, AbortSignal.timeout(30_000))
            await changed(action.providerId, 'Default provider updated.')
            return
          }
          if (action.kind === 'two-factor') {
            await setCommunicationProviderTwoFactor(
              action.providerId,
              action.enabled,
            ...
            await changed(
              action.providerId,
              action.enabled
                ? 'Provider now delivers two-factor codes.'
                : 'Provider removed from two-factor delivery.',
            )
            return
          }
          if (action.kind === 'test') {
            const provider = await sendCommunicationProviderTest(
              action.providerId,
              action.recipient,
              AbortSignal.timeout(30_000),
            )
            setMessage(`Test message sent via ${provider}.`)
            return
          }
          ...
          await updateCommunicationProvider(
            provider.id,
            { ...providerInput(providerDraft(provider)), enabled: action.enabled },
            AbortSignal.timeout(30_000),
          )
          await changed(
            provider.id,
            `Provider '${provider.name}' ${action.enabled ? 'enabled' : 'disabled'}.`,
          )
        } catch (error: unknown) {
  ...
  async function changeDraft(value: CommunicationProviderDraft): Promise<void> {
    await Promise.resolve()
    setDraft(value)
  }
  async function selectProject(projectId: string | null): Promise<void> {
    ...
    setSecrets(
      projectId === null
        ? []
        : await fetchCommunicationSecrets(projectId, AbortSignal.timeout(30_000)),
    )
  }
  async function editProvider(provider: CommunicationProvider): Promise<void> {
    ...
    setShowForm(true)
    setSecrets(
      value.vaultProjectId === null
        ? []
        : await fetchCommunicationSecrets(value.vaultProjectId, AbortSignal.timeout(30_000)),
    )
  }
  async function saveDraft(): Promise<void> {
    await eventBus.publish('settings.communication-save-requested', {
  ...
  return (
    <div className="settings-page communications-page">
      <title>placecontext — Communications</title>
      <header className="settings-page-head">
        <div>
          <span className="settings-kicker">Workspace integrations</span>
          <h1>Communications</h1>
          <p>
            Connect providers for workflows and sign-in codes. Credentials are referenced from
            project Vaults and resolved only when a message is sent.
          </p>
        </div>
        <button className="dcbtn primary" onClick={() => void openAdd()} type="button">
          + Add provider
        </button>
      </header>
      {message === null ? null : (
      ...
      <section className="dccard provider-card">
        <CommunicationProviderList
          busy={busy}
          onAction={async (action) =>
            eventBus.publish('settings.communication-action-requested', action)
          }
          onEdit={editProvider}
