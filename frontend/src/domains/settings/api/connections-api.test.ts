...
describe('connections API', () => {
  ...
  it('loads the composed context from the canonical api/v1 route', async () => {
    ...
    expect(fetchMock).toHaveBeenCalledWith(
      '/api/v1/settings/connections/context',
  ...
  it('writes and resets project connection settings', async () => {
    ...
    await saveExternalIndex(
      project.id,
      { endpoint: 'https://search.example.com', username: '', password: '', index: '*' },
      signal,
    )
    await resetExternalDatabase(project.id, signal)
    expect(fetchMock).toHaveBeenNthCalledWith(
      1,
      `/api/v1/settings/connections/projects/${project.id}/index`,
      expect.objectContaining({ method: 'PUT' }),
    )
    expect(fetchMock).toHaveBeenNthCalledWith(
      2,
      `/api/v1/settings/connections/projects/${project.id}/database`,
