...
export async function saveExternalDatabase(
  projectId: string,
  input: ExternalDatabaseInput,
  signal: AbortSignal,
): Promise<ConnectionProject> {
  return putJson({
    path: `${ROOT}/projects/${projectId}/database`,
    body: input,
    schema: connectionProjectSchema,
    signal,
  })
}

export async function resetExternalDatabase(
  projectId: string,
  signal: AbortSignal,
): Promise<ConnectionProject> {
  return deleteJson({
...
export async function saveExternalIndex(
  projectId: string,
  input: ExternalIndexInput,
  signal: AbortSignal,
): Promise<ConnectionProject> {
  return putJson({
    path: `${ROOT}/projects/${projectId}/index`,
    body: input,
    schema: connectionProjectSchema,
    signal,
  })
...
export async function resetExternalIndex(
  projectId: string,
  signal: AbortSignal,
): Promise<ConnectionProject> {
  return deleteJson({
