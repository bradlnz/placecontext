...
const apiTokenSchema = z.object({
  id: z.uuid(), name: z.string(), tokenPrefix: z.string(),
  createdAt: z.iso.datetime({ offset: true }),
  lastUsedAt: z.iso.datetime({ offset: true }).nullable(),
  expiresAt: z.iso.datetime({ offset: true }).nullable(),
})
const createdApiTokenSchema: z.ZodType<CreatedApiToken> = apiTokenSchema.extend({ rawToken: z.string() })
...
export async function createApiToken(name: string, lifetimeDays: number, signal: AbortSignal): Promise<CreatedApiToken> {
  return postJson({ path: '/api/v1/settings/api-tokens', body: { name, lifetimeDays }, schema: createdApiTokenSchema, signal })
}

export async function revokeApiToken(tokenId: string, signal: AbortSignal): Promise<void> {
  await deleteJson({ path: `/api/v1/settings/api-tokens/${tokenId}`, schema: revocationSchema, signal })
}
