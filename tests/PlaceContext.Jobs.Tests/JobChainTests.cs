...
public class JobChainTests
{
    ...
    public async Task Run_threads_each_steps_output_into_the_next_step()
    {
        ...
        // First step gets the caller's payload; the accumulated context passed to the second step
        // keeps the caller fields and merges in the first step's artifact.
        Assert.Equal("{\"from\":\"caller\"}", dispatcher.Payloads[0]);
        Assert.Equal("{\"from\":\"caller\",\"rows\":3}", dispatcher.Payloads[1]);
        Assert.Equal("{\"from\":\"caller\",\"rows\":3,\"report\":\"done\"}", view.FinalOutput);
    }
    ...
    public async Task Run_prefers_the_reduce_artifact_and_wraps_multiple_shards_as_a_json_array()
    {
        ...
        Assert.Equal("{\"total\":10,\"previous\":null}", dispatcher.Payloads[1]); // reduce wins over shards
        Assert.Equal(
            "{\"total\":10,\"previous\":null,\"branches\":[{\"n\":1},\"plain text\"]}",
            dispatcher.Payloads[2]); // shard array is retained in the accumulated context
    }
    ...
    public async Task Wait_gate_persists_state_and_resume_continues_the_same_run_without_delaying()
    {
        ...
        Assert.Equal("{\"first\":true,\"previous\":null}", dispatcher.Payloads[1]);
    ...
    public async Task Run_threads_every_branchs_output_into_the_join()
    {
        ...
        Assert.Equal("Succeeded", view.Status);
        // The join receives both branches in order inside the accumulated context envelope.
        var joinPayload = dispatcher.Payloads[dispatcher.DispatchedJobIds.IndexOf(join.Id)];
        Assert.Equal("{\"previous\":null,\"branches\":[{\"branch\":1},{\"branch\":2}]}", joinPayload);
        Assert.Equal("{\"previous\":null,\"branches\":[{\"branch\":1},{\"branch\":2}],\"joined\":true}", view.FinalOutput);
    }
