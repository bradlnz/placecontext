...
{
    private readonly CrmIngestionPayloadValidator _validator =
        new(new LeadIngestionRequestValidator());
    ...
    public async Task Contact_form_payload_uses_lead_validation_rules()
    {
        ...
        Assert.Contains(result.Errors, error => error.ErrorMessage == "Enter a valid email address.");
    }
    ...
    public async Task Null_json_is_rejected()
    {
        ...
        Assert.False(result.IsValid);
        Assert.Contains(result.Errors, error => error.ErrorMessage == "A JSON payload is required.");
    }
}
