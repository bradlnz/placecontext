...
public sealed class DataMapListContractTests
{
    ...
    public void Data_map_is_a_job_list_that_exposes_unmapped_outputs()
    {
        ...
        var page = File.ReadAllText(Path.Combine(root, "src", "PlaceContext.Host", "Components", "Pages", "DataMap.razor"));
        var css = File.ReadAllText(Path.Combine(root, "src", "PlaceContext.Host", "Components", "Pages", "DataMap.razor.css"));
        ...
        Assert.Contains("Completed runs do not populate a project table.", page, StringComparison.Ordinal);
