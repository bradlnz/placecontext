    ...
    public void Auto_linked_records_route_through_the_entity_mapped_to_the_table()
    {
        ...
        var viewModel = File.ReadAllText(
            Path.Combine(
                ...
                "ViewModels",
                "EntityBrowse",
                "EntityBrowseViewModel.cs"
            )
        );

        Assert.Contains("Vm.EntityNameForTable(link.TableName)", page, StringComparison.Ordinal);
        Assert.Contains("public string EntityNameForTable", viewModel, StringComparison.Ordinal);
    }
