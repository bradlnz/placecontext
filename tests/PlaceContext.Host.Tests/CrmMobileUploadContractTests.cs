...
public sealed class CrmMobileUploadContractTests
{
    ...
    public void Client_file_picker_stacks_on_mobile_with_a_full_width_touch_target()
    {
        ...
        Assert.Contains(".artifact-dropzone {\n        align-items: stretch;\n        flex-direction: column;", css, StringComparison.Ordinal);
        Assert.Contains(".dropzone-action {\n        width: 100%;\n        min-height: 44px;", css, StringComparison.Ordinal);
        Assert.Contains(".dropzone-copy small {\n        white-space: normal;", css, StringComparison.Ordinal);
    }
