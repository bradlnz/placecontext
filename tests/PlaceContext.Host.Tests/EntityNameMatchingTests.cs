...
{
    private static DataEntityView Entity(string name, string table) => new(
        Guid.NewGuid(), Guid.NewGuid(), name, table, "id",
        Array.Empty<EntityRelationDto>(), Array.Empty<string>(), DateTimeOffset.UtcNow);
    ...
    public void EntityNameMatches_name_table_and_slug(string name, string table, string request, bool expected)
