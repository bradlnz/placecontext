...
public class HandlerTests
{
    ...
    public async Task RecordActivity_appends_commits_and_attaches_sha()
    {
        ...
        var uow = new RecordingUnitOfWork();
        var handler = new RecordActivityHandler(repo, ledgers, new FakeGitPort { ShaToReturn = "deadbee" }, uow, new FakeClock(T0));

        var view = await handler.HandleAsync(new RecordActivityCommand(
