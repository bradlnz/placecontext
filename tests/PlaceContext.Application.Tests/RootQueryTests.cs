...
public class RootQueryTests
{
    ...
    private static async Task<(InMemoryProjectRepository, InMemoryChangeLedgerRepository, Project)> SeedAsync()
    {
        var projects = new InMemoryProjectRepository();
        var ledgers = new InMemoryChangeLedgerRepository();

        var p = Project.Discover(RepoPath.From("/home/brad/code/alpha"), ProjectName.From("alpha"), T0);
        p.Register(T0);
        p.ApplyDebt(DebtScore.From(0.4), DebtScore.From(0.6), T0);
    ...
    public async Task RootStats_counts_today_changes_by_actor()
    {
        ...
        ledger.Append("a", Author.Agent("claude"), Rationale.Of("x"), TestDelta.From(1, 0, 0),
            DebtDelta.None, new ChangeVerification(true, true), new[] { "f.cs" }, Array.Empty<GraphNodeId>(), T0);
        ledger.Append("b", Author.Human("brad"), Rationale.Of("y"), TestDelta.None,
            DebtDelta.None, ChangeVerification.None, new[] { "g.cs" }, Array.Empty<GraphNodeId>(), T0);
        ...
        Assert.Equal(0.6, stats.RootAgenticDebt, 3);
    }

    [Fact]
    public async Task RootLedger_flags_untrusted_agent_change_and_marks_clean_one()
    {
        var (projects, ledgers, p) = await SeedAsync();
        var ledger = await ledgers.GetForProjectAsync(p.Id);
        ...
        ledger.Append("clean patch", Author.Agent("claude"), Rationale.Of("why"), TestDelta.From(2, 0, 0),
            DebtDelta.None, new ChangeVerification(true, true), new[] { "b.cs" }, Array.Empty<GraphNodeId>(), T0);
        await ledgers.SaveAsync(ledger);

        var handler = new GetRootLedgerHandler(projects, ledgers, new AgenticDebtScorer());
        var view = await handler.HandleAsync(new GetRootLedgerQuery(10));
